...
#include "meshlist.h"
//----------------------------------------------------------------------
MeshList::MeshList(SimulationInfo *sinfo, Variables *vars, MDRect r) {
  ...
  ChangeScale(vars->GetSigma(), sinfo, r);
}
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
void
MeshList::ChangeScale(double sigma, SimulationInfo *sinfo, MDRect myrect) {
  if (NULL != mesh_index) delete [] mesh_index;
  if (NULL != mesh_index2) delete [] mesh_index2;
  if (NULL != mesh_particle_number) delete [] mesh_particle_number;
  ...
  int msx = static_cast<int>(wx / SL / sigma);
  int msy = static_cast<int>(wy / SL / sigma);
  int msz = static_cast<int>(wz / SL / sigma);
